...
const easeOutExponential = (x: number) => 1 - Math.pow(1 - x, 2.5);
...
export const MoonIcon = ({ lat, phase }: MoonIconProps) => {
	...
	const scale = useMemo(() => {
		...
		if (phase < 0.25) {
			return min + easeInOutQuad(phase / 0.25) * (max - min);
		} else if (phase < 0.5) {
			return max - easeOutExponential((phase - 0.25) / 0.25) * (max - min);
		} else if (phase < 0.75) {
			return min + easeInOutQuad((phase - 0.5) / 0.25) * (max - min);
		} else {
			return max - easeOutExponential((phase - 0.75) / 0.25) * (max - min);
		}
	}, [phase]);
	...
	const translation = useMemo(() => {
		if (phase < 0.25) {
			...
			return max - easeInOutQuad((phase - 0.25) / 0.25) * (max - min);
		} else if (phase < 0.75) {
			const min = 64;
			const max = 68;
			...
			return max - easeInOutQuad((phase - 0.75) / 0.25) * (max - min);
		}
	}, [phase]);
